...
class N_keterampilan extends Master {
    ...
    public function import($bawa) {
        ...
        $detil_guru = $this->db->query("SELECT 
                                         ...
                                         WHERE b.id = ".$pc_bawa[0]." AND c.id = ".$pc_bawa[1]." AND a.tasm = '".$this->d['c']['ta_tasm']."'")->row_array();
        ...
        $q_siswa_kelas = $this->db->query("SELECT a.id_siswa, b.nama nmsiswa FROM t_kelas_siswa a LEFT JOIN m_siswa b ON a.id_siswa = b.id WHERE a.id_kelas = '".$pc_bawa[1]."' AND a.ta = '".$this->d['c']['ta_tahun']."' ORDER BY b.nama ASC")->result_array();
    ...
    public function cetak($bawa,$tasm=0) {
        $tasm = $tasm == 0 ? $this->d['c']['ta_tasm'] : $tasm;
