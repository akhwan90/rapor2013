			...
			if (!empty($data)) {
				...
				foreach ($data as $d) {
					...
					if ($size_array > 3) {
						...
						$html .= '<td class="ctr">-</td>';
						$teks_deskripsi[] = "-";
					}

					$nilai_akhir = ($jml > 0) ? ($porsi_h*($total/$jml))+($porsi_t*$d['nilai_uts'])+($porsi_a*$d['nilai_uas']) : ($porsi_t*$d['nilai_uts'])+($porsi_a*$d['nilai_uas']);

					$nilai_huruf = nilai_huruf($nilai_akhir);
					$nilai_deskripsi = '<span style="font-size: 10px">'.implode(", ", $teks_deskripsi).'</span>';

					$rt_rt = $total > 3 ? number_format($total/$jml) : 0;

					$html .= '<td class="rgt">'.$rt_rt.'</td><td class="rgt">'.$d['nilai_uts'].'</td><td class="rgt">'.$d['nilai_uas'].'</td><td class="rgt">'.number_format($nilai_akhir).'</td><td class="ctr">'.$nilai_huruf.'</td><td>'.$nilai_deskripsi.'</td></tr>';
