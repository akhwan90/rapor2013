...
-- Dumping structure for procedure nilai_mts.n_pengetahuan
DELIMITER //
CREATE DEFINER=`mtsnsid2`@`localhost` PROCEDURE `n_pengetahuan`(IN `id_mapel` INT, IN `id_kelas` INT)
BEGIN
	SET @sql = NULL;
...
-- Dumping structure for procedure nilai_mts.r_nilai_keterampilan
DELIMITER //
CREATE DEFINER=`mtsnsid2`@`localhost` PROCEDURE `r_nilai_keterampilan`(IN `id_mapel` INT, IN `id_kelas` INT)
BEGIN
	SET @sql = NULL;
